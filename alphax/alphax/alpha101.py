...
class Alpha101(object):
    ...
    def alpha_3(self, data, param1=10, param2=-1, dependencies=['open_price_raw', 'turnover_vol'], max_window=11):
        # -1 * correlation(rank(OPEN), rank(VOLUME), 10)

        rank_open_df = data['open_price_raw'].rank(axis=1, pct=True)
    ...
    def alpha_4(self, data, param1=9, param2=-1, dependencies=['lowest_price_raw'], max_window=10):
        # -1 * ts_rank(rank(LOW), 9)

        rank_low = data['lowest_price_raw'].rank(axis=1, pct=True)
        ts_win = param1
    ...
    def alpha_5(self, data, param1=10, dependencies=['open_price_raw', 
                                                     'close_price_raw', 'vwap_raw'], max_window=10):
        # rank((OPEN - (sum(VWAP, 10) / 10))) * (-1 * abs(rank((CLOSE - VWAP))))

        mean_win = param1
        open_vwap = data['open_price_raw'] - data['vwap_raw'].rolling(window=mean_win).mean()
        rank_open = open_vwap.rank(axis=1, pct=True)
        abs_rank = (data['close_price_raw'] - data['vwap_raw']).rank(axis=1, pct=True).abs() * (-1.0)
        alpha = (rank_open * abs_rank).iloc[-1]
        return alpha
    
    def alpha_6(self, data, param1=10, param2=-1, dependencies=['open_price', 'turnover_vol'], max_window=11):
    ...
    def alpha_11(self, data, param1=3, param2=3, 
                 dependencies=['close_price_raw', 'vwap_raw', 'turnover_vol'], max_window=5):
    # (rank(ts_max((VWAP - CLOSE), 3)) + rank(ts_min((VWAP - CLOSE), 3))) * rank(delta(VOLUME, 3))

        ts_max = (data['vwap_raw'] - data['close_price_raw']).rolling(window=param1).max()
        ts_min = (data['vwap_raw'] - data['close_price_raw']).rolling(window=param1).min()
    ...
    def alpha_12(self, data, param1=1, param2=-1, dependencies=['close_price_raw', 'turnover_vol'], max_window=2):
        # sign(delta(VOLUME, 1)) * (-1 * delta(CLOSE, 1))

        alpha = np.sign(data['turnover_vol'].diff(periods=param1)) * data['close_price_raw'].diff(periods=param1)
        alpha = alpha.iloc[-1] * float(param2)
        return alpha
    
    def alpha_13(self, data, param1=5, param2=-1, dependencies=['close_price_raw', 'turnover_vol'], max_window=6):
    # -1 * rank(covariance(rank(CLOSE), rank(VOLUME), 5))

        rank_close_df = data['close_price_raw'].rank(axis=1, pct=True)
        rank_volume_df = data['turnover_vol'].rank(axis=1, pct=True)
    ...
    def alpha_14(self, data, param1=10, param2=3, param3=-1, 
    ...
    def alpha_15(self, data, param1=3, param2=3, param3=-1,
                 dependencies=['highest_price_raw', 'turnover_vol'], max_window=6):
    # -1 * sum(rank(correlation(rank(HIGH), rank(VOLUME), 3)), 3)

        rank_high_df = data['highest_price_raw'].rank(axis=1, pct=True)
        rank_volume_df = data['turnover_vol'].rank(axis=1, pct=True)
    ...
    def alpha_16(self, data, param1=5, param2=-1,
                 dependencies=['highest_price_raw', 'turnover_vol'], max_window=6):
    ...
    def alpha_18(self, data, param1=10, param2=5, param3=-1,
                 dependencies=['open_price_raw', 'close_price_raw'], max_window=10):
    # -1 * rank((stddev(abs((CLOSE - OPEN)), 5) + (CLOSE - OPEN)) + correlation(CLOSE, OPEN, 10))

        corr_win = param1
        id_begin = data['open_price_raw'].index[-corr_win]
        corr_se = data['open_price_raw'].loc[id_begin:].corrwith(data['close_price_raw'].loc[id_begin:])

        price = data['close_price_raw'] - data['open_price_raw']
        price_se = (price.abs().rolling(window=param2).std() + price).iloc[-1]

        alpha = float(param3) * (price_se + corr_se).rank(pct=True)
        return alpha
    
    def alpha_19(self, data, param1=7, param2=70, param3=1, param4=-1,
                 dependencies=['close_price_raw', 'returns'], max_window=70):
    # -1 * sign(((CLOSE - delay(CLOSE, 7)) + delta(CLOSE, 7))) * (1 + rank(1 + sum(RETURNS, 250)))

        sign_se = np.sign(data['close_price_raw'].diff(param1)).iloc[-1]
    ...
    def alpha_22(self, data, param1=5, param2=20, param3=-1, 
                 dependencies=['close_price', 'highest_price_raw', 'turnover_vol'], max_window=20):
        # -1 * (delta(correlation(HIGH, VOLUME, 5), 5) * rank(stddev(CLOSE, 20)))

        corr_df = rolling_corr(data['highest_price_raw'], data['turnover_vol'], win=param1)
    ...
    def alpha_23(self, data, param1=20, param2=-1, param3=2, param4=0.25,
                 dependencies=['highest_price', 'highest_price_raw'], max_window=20):
        ...
        delta_high = float(param2) * data['highest_price_raw'].diff(param3)
        delta_high[mark==False] = delta_high[mark==False] * param4
        alpha = delta_high.iloc[-1]
        return alpha
    
    def alpha_24(self, data, param1=40, param2=20, param3=6, param4=-1,
                 dependencies=['close_price', 'close_price_raw'], max_window=70):
        ...
        true_se = float(param4) * data['close_price_raw'].diff(periods=param3).iloc[-1]
        false_se = float(param4) * (data['close_price_raw'] - data['close_price_raw'].rolling(
    ...
    def alpha_26(self, data, param1=10, param2=10, param3=5, param4=-1,
    ...
    def alpha_27(self, data, param1=10, param2=2, param3=-1,
    ...
    def alpha_29(self, data, param1=5, param2=4, param3=3,param4=-1,param5=6,param7=20,
                 dependencies=['close_price_raw', 'returns'], max_window=30):
        # # original formula
        ...
        df = (float(param4) * data['close_price_raw'].diff(periods=param1).rank(axis=1, pct=True)).rank(axis=1, pct=True)
    ...
    def alpha_32(self, data, param1=7, param2=40, param3=5, param4=20,
                 dependencies=['close_price_raw', 'close_price', 'vwap'], max_window=50):
        # # original formula
        ...
        close_se = (data['close_price_raw'].rolling(window=param1).mean() - data['close_price_raw']).iloc[-1]
    ...
    def alpha_36(self, data, param1=15, param2=6, param3=10, param4=20, param5=50,
                 ...
                 dependencies=['close_price_raw', 'open_price_raw', 'close_price', 
                               'open_price', 'vwap', 'turnover_vol', 'returns'], max_window=60):
        # # original formula
        ...
        # rank(OPEN - CLOSE)
        part2 = (data['open_price_raw'] - data['close_price_raw']).iloc[-1].rank(pct=True)
        ...
        sum_close = data['close_price_raw'].rolling(window=sum_win).mean() - data['open_price_raw']
        close_open = data['close_price_raw'] - data['open_price_raw']
        part5 = (sum_close * close_open).iloc[-1].rank(pct=True)

        alpha = param6 * part1 + param7 * part2 + param8 * part3 + part4 + param9 * part5
        return alpha
    
    def alpha_40(self, data, param1=10, param2=10, param3=-1,
                 dependencies=['highest_price_raw', 'highest_price', 'turnover_vol'], max_window=12):
        # (-1 * rank(stddev(HIGH, 10))) * correlation(HIGH, VOLUME, 10)

        part1 = float(param3) * data['highest_price_raw'].rolling(window=param1).std().iloc[-1].rank(pct=True)
        corr_win = param2
    ...
    def alpha_44(self, data, param1=5, param2=-1,
    ...
    def alpha_45(self, data, param1=5, param2=20, param3=6, param4=6, param5=5, param6=20, param7=-1,
                 dependencies=['close_price_raw', 'close_price', 'turnover_vol'], max_window=30):
        ...
        part1 = data['close_price_raw'].shift(periods=param1).rolling(window=param2).mean().iloc[-1].rank(pct=True)
    ...
    def alpha_50(self, data, param1=5, param2=5, param3=-1,
                 dependencies=['vwap_raw', 'turnover_vol'], max_window=10):
        # -1 * ts_max(rank(correlation(rank(VOLUME), rank(VWAP), 5)), 5)

        rank_vwap_df = data['vwap_raw'].rank(axis=1, pct=True)
        rank_volume_df = data['turnover_vol'].rank(axis=1, pct=True)
    ...
    def alpha_52(self, data, param1=8, param2=8, param3=80, param4=8, param5=8,
                 dependencies=['lowest_price_raw', 'returns', 'turnover_vol'], max_window=10):
        # (-ts_min(LOW, 5) + delay(ts_min(LOW, 5), 5)) *
        # rank(((sum(RETURNS, 240) - sum(RETURNS, 20)) / 220)) * ts_rank(VOLUME, 5)

        # (-ts_min(LOW, 5) + delay(ts_min(LOW, 5), 5))
        ts_max_win = param1
        id_begin = data['lowest_price_raw'].index[-ts_max_win]
        part1 = data['lowest_price_raw'].shift(periods=param2
                                              ).loc[id_begin:].min() - data['lowest_price_raw'].loc[id_begin:].min()
    ...
    def alpha_53(self, data, param1=2, param2=9, param3=0.001,
    ...
    def alpha_54(self, data, param1=0.001, param2=5, param3=4, param5=-1,
    ...
    def alpha_55(self, data, param1=12, param2=12, param3=6, param4=-1,
    ...
    def alpha_57(self, data, param1=2, param2=30, param3=4, param4=-1,
                 dependencies=['close_price_raw', 'vwap_raw', 'close_price'], max_window=35):
        # -1 * (CLOSE - VWAP) / decay_linear(rank(ts_argmax(CLOSE, 30)), 2)

        # (CLOSE - VWAP)
        ma_win = param1
        numerator = (data['close_price_raw'] - data['vwap_raw']).iloc[-ma_win:].mean()
    ...
    def alpha_59(self, data, param1=0.7, param2=0.3, param3=9, param4=12, param5=10, param6=-1,
                 dependencies=['vwap_raw', 'close_price_raw', 'turnover_vol'], max_window=30):
        ...
        # indneutralize(VWAP, indclass)
        neu_df = df_indneutralize(data['vwap_raw'] * param1 + data['close_price_raw'] * param2, data['indu'])
    ...
    def alpha_62(self, data, param1=20, param2=10, param3=10, param4=2, param5=-1,
                 dependencies=['turnover_vol', 'vwap', 'open_price_raw', 'highest_price_raw', 
                               'lowest_price_raw'], max_window=40):
        ...
        # rank(((rank(OPEN) * 2) - (rank(((HIGH + LOW) / 2)) + rank(HIGH))))
        rank_open = data['open_price_raw'].rank(axis=1, pct=True)
        rank_high_low = ((data['highest_price_raw'] + data['lowest_price_raw']) / float(param4)).rank(axis=1, pct=True)
        rank_high = data['highest_price_raw'].rank(axis=1, pct=True)
        part2 = (rank_open - rank_high_low - rank_high).rank(axis=1, pct=True).iloc[-1]

        alpha = float(param5) * (part1 - part2)
        return alpha
    
    def alpha_66(self, data, param1=0.001, param2=2, param3=12, param4=7, param5=-1,
                 dependencies=['vwap_raw', 'lowest_price', 'vwap', 'open_price', 'highest_price'], max_window=20):
        ...
        part1 = decay_linear(data['vwap_raw'].diff(periods=4), win=8).rank(pct=True)
    ...
    def alpha_67(self, data, param1=20, param2=10, param3=5, param4=8, param5=-1,
                 dependencies=['highest_price_raw', 'vwap_raw', 'turnover_vol'], max_window=30):
        ...
        high_df = data['highest_price_raw'] - data['highest_price_raw'].rolling(window=param1).min()
        part1 = high_df.rank(axis=1, pct=True).iloc[-param2:].mean()

        # rank(correlation(indneutralize(VWAP, indclass), indneutralize(ADV20, indclass), 6.02936))
        neu_vwap = df_indneutralize(data['vwap_raw'], data['indu'])
    ...
    def alpha_69(self, data, param1=3, param2=5, param3=8, param4=-1,
                 dependencies=['vwap_raw', 'turnover_vol'], max_window=15):
    ...
    def alpha_72(self, data, param1=5, param2=1.0e6, param3=9, param4=10, param5=2,
    ...
    def alpha_73(self, data, param1=5, param2=3, param3=0.147155, param4=0.147155,
                 param5=2, param6=4, param7=17,param8=-1,param9=-1,
                 dependencies=['vwap_raw', 'lowest_price', 'open_price'], max_window=25):
        ...
        part1 = decay_linear(data['vwap_raw'].diff(periods=diff_win), win=decay_win).rank(pct=True)
    ...
    def alpha_74(self, data, param1=10, param2=16, param3=15, param4=0.0261661, param5=12, param6=-1,
                 dependencies=['turnover_vol', 'close_price', 'highest_price_raw', 'vwap_raw'], max_window=40):
        ...
        # rank(correlation(rank(HIGH * 0.0261661 + VWAP * (1 - 0.0261661)), rank(VOLUME), 11.4791))
        rank_price = (data['highest_price_raw'] * param4 + data['vwap_raw'] * (1 - param4)).rank(axis=1, pct=True)
        rank_vol = data['turnover_vol'].rank(axis=1, pct=True)
    ...
    def alpha_75(self, data, param1=8, param2=12, param3=12,
                 dependencies=['vwap', 'turnover_vol', 'lowest_price_raw', 'turnover_vol'], max_window=30):
        ...
        rank_price = data['lowest_price_raw'].rank(axis=1, pct=True)
    ...
    def alpha_76(self, data, param1=5, param2=1, param3=5, param4=8, param5=20, param6=5,
                 ...
                 dependencies=['close_price_raw', 'vwap_raw', 'lowest_price_raw', 'turnover_vol'], 
                 max_window=50):
        ...
        neu_low = df_indneutralize(data['lowest_price_raw'], data['indu'])
        ...
        decay_df = rolling_decay(data['vwap_raw'].diff(periods=diff_win), win=decay_win)
    ...
    def alpha_80(self, data, param1=0.85, param2=0.15, param3=5, param4=4, param5=5,
                 ...
                 dependencies=['open_price_raw', 'highest_price_raw', 'turnover_vol', 'highest_price'], max_window=20):
        ...
        neu_price = df_indneutralize(data['open_price_raw'] * param1 + data['highest_price_raw'] * param2, data['indu'])
    ...
    def alpha_81(self, data, param1=10, param2=10, param3=8, param4=10, param5=4,
                 param6=8, param7=-1,
                 dependencies=['vwap', 'turnover_vol', 'vwap_raw'], max_window=32):
        ...
        # rank(correlation(rank(VWAP), rank(VOLUME), 5.07914))
        rank_price = data['vwap_raw'].rank(axis=1, pct=True)
        rank_vol = data['turnover_vol'].rank(axis=1, pct=True)
    ...
    def alpha_82(self, data, param1=1, param2=10, param3=16, param4=6, param5=14, param6=-1, param7=0.5,
                 dependencies=['open_price_raw', 'turnover_vol'], max_window=40):
        ...
        diff_win, decay_win = param1, param2
        part1 = decay_linear(data['open_price_raw'].diff(periods=diff_win), win=decay_win).rank(pct=True)

        # ts_rank(decay_linear(correlation(indneutralize(VOLUME, indclass), OPEN, 17.4842), 6.92131), 13.4283)
        neu_vol = df_indneutralize(data['turnover_vol'], data['indu'])
        corr_win, decay_win, ts_rank_win = param3, param4, param5
        decay_df = rolling_decay(rolling_corr(neu_vol, data['open_price_raw'], win=corr_win), win=decay_win)
        part2 = decay_df.iloc[-ts_rank_win:].rank(axis=0, pct=True).iloc[-1]

        alpha_df = pd.DataFrame({'part1': part1, 'part2': part2})

        # # original alpha formula
        # alpha = -1.0 * alpha_df.min(axis=1)
        # adjusted alpha formula
        alpha = float(param6) * (alpha_df.min(axis=1) - float(param7) * alpha_df.max(axis=1))
        return alpha
    
    def alpha_83(self, data, param1=10, param2=2,
                 dependencies=['highest_price', 'lowest_price', 
                               'close_price', 'turnover_vol', 
                               'vwap_raw', 'close_price_raw'], max_window=20):
        ...
        part2 = (data['turnover_vol'].rank(axis=1, pct=True) * (data['vwap_raw'] - data['close_price_raw'])).iloc[-1]
    ...
    def alpha_84(self, data, param1=15, param2=20, param3=6,
                 dependencies=['vwap', 'close_price_raw'], max_window=40):
        ...
        # delta(CLOSE, 4.96796)
        diff_win = param3
        part2 = data['close_price_raw'].diff(periods=diff_win).iloc[-1]
        part2 = data['close_price_raw'].diff(periods=diff_win).iloc[-1].rank(pct=True)
    ...
    def alpha_87(self, data, param1=2, param2=3, param3=0.37, param4=0.63, param5=12, param6=5,
                 param7=14,
                 dependencies=['close_price_raw', 'vwap_raw', 'turnover_vol'], max_window=30):
        ...
        diff_win, decay_win = param1, param2
        price_df = data['close_price_raw'] * param3 + data['vwap_raw'] * param4
        ...
        decay_df = rolling_decay(rolling_corr(neu_adv, data['close_price_raw'], win=corr_win).abs(), win=decay_win)
        part2 = decay_df.iloc[-ts_rank_win:].rank(axis=0, pct=True).iloc[-1]
    
        alpha_df = pd.DataFrame({'part1': part1, 'part2': part2})
    
        # # original alpha formula
        # alpha = -1.0 * alpha_df.max(axis=1)
        # adjusted alpha formula
        alpha = -1.0 * (alpha_df.max(axis=1) - 0.5 * alpha_df.min(axis=1))
        return alpha
    
    def alpha_88(self, data, param1=8, param2=20, param3=9, param4=20, param5=9, param6=6, param7=20,
                 dependencies=['open_price_raw', 'highest_price_raw', 
                               'lowest_price_raw', 'close_price_raw', 
                               'turnover_vol', 'close_price'], max_window=50):
        ...
        decay_win = param1
        open_low = data['open_price_raw'].rank(axis=1, pct=True) + data['lowest_price_raw'].rank(axis=1, pct=True)
        high_close = data['highest_price_raw'].rank(axis=1, pct=True) + data['close_price_raw'].rank(axis=1, pct=True)
    ...
    def alpha_90(self, data, param1=5, param2=4, param3=8, param4=8, param5=6, param6=-1,
                 dependencies=['close_price_raw', 'lowest_price_raw', 
        ...
        # add a decay linear
        close_df = data['close_price_raw'] - data['close_price_raw'].rolling(window=param1).max()
        part1 = decay_linear(close_df.rank(axis=1, pct=True), win=param2).rank(pct=True)
    
        # ts_rank(correlation(indneutralize(ADV40, indclass), LOW, 5.38375), 3.21856)
        neu_adv = df_indneutralize(data['turnover_vol'].rolling(window=param3).mean(), data['indu'])
        corr_win, ts_rank_win = param4, param5
        corr_df = rolling_corr(neu_adv, data['lowest_price_raw'], win=corr_win)
        part2 = corr_df.iloc[-ts_rank_win:].rank(axis=0, pct=True).iloc[-1]
    ...
    def alpha_91(self, data, param1=5, param2=10, param3=10, param4=3, param5=10, 
                 ...
                 dependencies=['close_price_raw', 'turnover_vol', 
                               'vwap'], max_window=32):
        ...
        neu_close = df_indneutralize(data['close_price_raw'], data['indu'])
        adv = data['turnover_vol'].rolling(window=param1).mean()
    ...
    def alpha_96(self, data, param1=6, param2=4, param3=14, param4=20, param5=8, param6=6,
                 param7=8, param8=12, param9=6, param10=13, param11=-1,
                 dependencies=['vwap_raw', 'turnover_vol', 
        ...
        rank_vwap = data['vwap_raw'].rank(axis=1, pct=True)
        rank_vol = data['turnover_vol'].rank(axis=1, pct=True)
    ...
    def alpha_97(self, data, param1=4, param2=12, param3=0.7, param4=10, param5=17,
                 ...
                 dependencies=['lowest_price_raw', 'vwap_raw', 'turnover_vol', 'lowest_price'], max_window=45):
        ...
        # rank(decay_linear(delta(indneutralize(((LOW * 0.721001) + (VWAP * (1 - 0.721001))), indclass), 3.3705), 20.4523))
        diff_win, decay_win = param1, param2
        price_df = data['lowest_price_raw'] * param3 + data['vwap_raw'] * (1 - param3)
    ...
    def alpha_99(self, data, param1=20, param2=16, param3=16, param4=9, param5=2, param6=7, param7=-1,
